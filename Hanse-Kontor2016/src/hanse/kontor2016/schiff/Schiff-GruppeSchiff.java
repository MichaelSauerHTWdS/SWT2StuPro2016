...
public class Schiff {
	...
	{
		//GeoObjekt position, Route route 
		this.name = name;
	...
	public static void wareAbladen(Schiff schiff , Ware ware) {
		if (schiff.schiffsware.contains(ware.name)) {
			int index = schiff.schiffsware.indexOf(ware.name);
			schiff.schiffsware.get(index).menge -= ware.menge;
			if (schiff.schiffsware.get(index).menge == 0)
				schiff.schiffsware.remove(ware);
		}
		//else --> Ware nicht gefunden
	}
